...
# Correctif complet pour numpy + transformers (Option A) à insérer AU DÉBUT de la cellule 6
fix_lines = [
...
already_has_marker = any('NumPy version:' in (s if isinstance(s, str) else '') for s in source)

if not already_has_marker:
    # Remplacer complètement le contenu de la cellule 6
    new_source = fix_lines + (['\n\n# --- Code existant de la cellule 6 ---\n'] if source else []) + source
    cell6['source'] = new_source
    cells[5] = cell6
    print('Correctif numpy+transformers inséré au début de la cellule 6.')
else:
